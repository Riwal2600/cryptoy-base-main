...
def encrypt_optimized(msg: str, a: int, b: int) -> str:
    ...
    return unicodes_to_str(a * x + b for x in str_to_unicodes(msg))
...
def decrypt_optimized(msg: str, a_inverse: int, b: int) -> str:
    ...
    # a la fonction
    return unicodes_to_str(a_inverse * (y - b) for x in str_to_unicodes(msg))
...
def attack_optimized() -> tuple[str, tuple[int, int]]:
    ...
    compute_affine_keys = compute_affine_keys(0x110000)

    for b in range(1, 10000):
        a_inverse = compute_affine_key_inverse(a, compute_affine_keys, 0x110000)
        for a in compute_affine_keys(0x110000):
